...
class veilingController extends Controller {
        ...
        if(isset($_GET['veiling'])) {
			...
			*/
				...
				if(isset($_POST['bod']) && !empty($_POST['bod'])) {
					...
					if (isset($_SESSION['loggedIn'])) {
						...
						if(!is_numeric($_POST['bod'])){
                            ...
                            if($floatBod < $startPrijs){
                                $data['error_input'] = "invalid_bod";
                            } else {

                                if($hoogsteBod <= $minWaardes['w1'] || empty($hoogsteBod)){
            ...
            if(empty($result)) {
                ...
                $data['biedingen'] = $this->generateBidHistory($data['bod']);
    ...
    function generateBidHistory($biddings){
        $bid_html = '';
